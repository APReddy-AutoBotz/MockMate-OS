...
-- Keep the database session initializer aligned with the same authoritative
-- mode policies used by the adaptive engine. For grounded sessions p_setup has
-- already been proven byte/logically equivalent to the persisted plan.
CREATE OR REPLACE FUNCTION public.initialize_interview_session_policy_tx() RETURNS TRIGGER AS $$
DECLARE v_mode TEXT:=COALESCE(NEW.setup#>>'{controls,reasoningMode}',NEW.setup#>>'{interviewPlan,meta,controls,reasoningMode}','classic_behavioral');
DECLARE v_initial_stage TEXT;
BEGIN
  v_initial_stage:=CASE WHEN v_mode IN ('classic_technical','problem_framing','ai_collaboration_review','uncertainty_handling') THEN 'clarification' ELSE 'framing' END;
  NEW.current_stage:=v_initial_stage;
  RETURN NEW;
END; $$ LANGUAGE plpgsql SET search_path=public,pg_temp;

DROP TRIGGER IF EXISTS initialize_interview_session_policy ON public.interview_sessions;
CREATE TRIGGER initialize_interview_session_policy
BEFORE INSERT ON public.interview_sessions FOR EACH ROW
EXECUTE FUNCTION public.initialize_interview_session_policy_tx();
